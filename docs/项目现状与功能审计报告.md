...
| `frontend/.env.development` | 开发环境 API 代理目标 | 转发到本机后端 `127.0.0.1:8000` |
| `frontend/.env.production` | Compose 前端的 API 基址 | 留空，使用同源 `/api` 并由容器 Nginx 转发 |
...
| `infra/nginx/local.conf` | 本地静态页面和 `/api` 反代配置 | 使用本机地址，不依赖域名和证书 |
